...
void handleClient(SOCKET clientSocket, sockaddr_in clientAddr, std::ofstream& logFile) {
    char clientIP[16];
...
int main() {
    ...
    while (true) {
        ...
        clientThreads.emplace_back([clientSocket, clientAddr, &logFile]() {
            handleClient(clientSocket, clientAddr, logFile);
        });
    }
    ...
    closesocket(serverSocket);
    WSACleanup();
    writeToLog("Сервер завершил работу", logFile);
    logFile.close();
